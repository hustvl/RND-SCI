...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        res = self.body(x).mul(
            self.res_scale)  # So res_scale is a scaler? just scale all elements in each feature's residual? Why?
        res += x
...
class EFF(nn.Module):
    ...
    def __init__(self, nin: int, nout: int, num_splits: int) -> None:
        ...
        self.subspaces = nn.ModuleList([DSC(int(self.nin / self.num_splits)) for i in range(self.num_splits)])
...
class HDNet(nn.Module):
    ...
    def forward(self, y, x, input_mask=None):
        x = self.head(x)
