...
class HS_MSA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.only_local_branch:
            ...
            # local branch
            q1, k1, v1 = map(lambda t: rearrange(t, 'b (h b0) (w b1) c -> b (h w) (b0 b1) c', b0=w_size[0], b1=w_size[1]),
                             (q1, k1, v1))
            ...
            # non-local branch
            q2, k2, v2 = map(lambda t: rearrange(t, 'b (h b0) (w b1) c -> b (h w) (b0 b1) c', b0=w_size[0], b1=w_size[1]),
                             (q2, k2, v2))
        ...
        for _ in range(num_blocks):
            self.blocks.append(
                nn.ModuleList([
                    PreNorm(
                        dim,
                        HS_MSA(dim=dim, window_size=window_size, dim_head=dim_head, heads=heads,
...
class DAUHST(nn.Module):
    ...
    def __init__(self, num_iterations=1):
        ...
        for _ in range(num_iterations):
            self.denoisers.append(HST(in_dim=29, out_dim=28, dim=28, num_blocks=[1, 1, 1]), )
