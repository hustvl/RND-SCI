...
class SAH_MSA(nn.Module):
    def __init__(self, heads=4, n_rounds=2, channels=64, patch_size=144, r=1):
        ...
        self.clustering_params = {'r': r, 'n_rounds': self.n_rounds}
...
class SAHAB(nn.Module):
    def __init__(self, dim, patch_size=(16, 16), heads=8, shift_size=0, sparse=False):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        if self.sparse:
            ...
            x = x.view(b, N, -1)
        x = rearrange(x,
                      'b (nh nw) (hh ww c) -> b (nh hh) (nw ww) c',
                      nh=h // (w_size[0] * 2),
                      hh=w_size[0] * 2,
                      ww=w_size[1] * 2)
...
class SAHABs(nn.Module):
    def __init__(self, dim, patch_size=(8, 8), heads=8, num_blocks=2, sparse=False):
        super().__init__()
        blocks = []
...
class ASPPConv(nn.Sequential):
    def __init__(self, in_channels, out_channels, dilation):
        modules = [nn.Conv2d(in_channels, out_channels, 3, padding=dilation, dilation=dilation, bias=False), nn.ReLU()]
...
class ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, out_channels):
        ...
        self.project = nn.Sequential(nn.Conv2d(len(self.convs) * out_channels, out_channels, 1, bias=False), nn.ReLU(),
                                     nn.Dropout(0.5))
...
class CST(nn.Module):
    def __init__(self, dim=28, stage=2, num_blocks=[2, 2, 2], sparse=False):
        ...
        # Bottleneck
        self.bottleneck = SAHABs(dim=dim_stage, heads=dim_stage // dim, num_blocks=num_blocks[-1], sparse=sparse)
