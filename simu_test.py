...
from utils import simu_par_args, torch_psnr, torch_ssim, model_input_setting
...
def test(model):
    ...
    y, input_meas = init_meas(test_gt, mask3d_batch, opt.input_setting)
    model.eval()
    with torch.no_grad():
        model_out = model(y, input_meas, input_mask)
