...
def generate_masks(mask_path, batch_size):
    ...
    mask3d_batch = mask3d.repeat([batch_size, 1, 1, 1]).cuda().float()
    return mask3d_batch
...
def generate_shift_masks(mask_path, batch_size):
    ...
    Phi_batch = mask_3d_shift.repeat([batch_size, 1, 1, 1]).cuda().float()
    Phi_s_batch = torch.sum(Phi_batch**2, 1)
    return Phi_batch, Phi_s_batch


def generate_rnd_masks(mask_path, batch_size=1):
    mask = sio.loadmat(mask_path + '/mask_3d_shift.mat')
    mask_3d_shift = mask['mask_3d_shift']
    mask_3d_shift = np.transpose(mask_3d_shift, [2, 0, 1])
    mask_3d_shift = torch.from_numpy(mask_3d_shift)
    ...
    Phi_batch = mask_3d_shift.repeat([batch_size, 1, 1, 1]).cuda().float()
    #--- remove other value ------
    Phi_batch = shift_back_3d_cuda(Phi_batch)
    Phi_batch[:, :, :, H:] = 1e-7
    Phi_batch = shift_3d_cuda(Phi_batch)
    #-----------
    Phi_s_batch = torch.sum(Phi_batch**2, 1)
    return Phi_batch, Phi_s_batch
...
def init_meas(gt, mask, input_setting):
    if input_setting == 'H':
        y, input_meas = gen_meas_torch(gt, mask, Y2H=True, mul_mask=False)
    elif input_setting == 'HM':
        y, input_meas = gen_meas_torch(gt, mask, Y2H=True, mul_mask=True)
    elif input_setting == 'Y':
        y, input_meas = gen_meas_torch(gt, mask, Y2H=False, mul_mask=True)
    return y, input_meas
