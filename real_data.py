...
def prepare_data_cave(path, file_num):
    ...
    for idx in range(file_num):
        ...
        if "img_expand" in data:
            img = np.ascontiguousarray(data['img_expand']) / 65536.
        elif "img" in data:
            img = np.ascontiguousarray(data['img']) / 65536.
        #------
...
def generate_rnd_masks(path, size=660):
    ## load mask
    data = sio.loadmat(path)
    mask = np.ascontiguousarray(data['mask'])
    mask_3d = np.tile(mask[:, :, np.newaxis], (1, 1, 28))
    mask_3d_shift = np.zeros((size, size + (28 - 1) * 2, 28)) + 1e-7
...
class dataset(tud.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_3d_shift = np.zeros((self.size, self.size + (28 - 1) * 2, 28)) + 1e-7
        ...
        mask_3d_shift = torch.FloatTensor(mask_3d_shift.copy()).permute(2, 0, 1)
        mask_3d_shift_s = torch.FloatTensor(mask_3d_shift_s.copy())
        return torch.FloatTensor(input.clone().detach()), input, label, mask_3d, mask_3d_shift, mask_3d_shift_s
